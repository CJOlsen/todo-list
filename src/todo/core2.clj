...
(defn get-list [list-name]
  ;; try to load current list data from memory
...
(defn startup-lists []
  ...
  (map #(make-todo % "none") (get-active-list-names)))
        ...
        :action (fn [e]
                  (let [;; active-listbox (listbox :model
                        ...
                        add-new-dialog (dialog :content
                                               (flow-panel :items
                                                           ["add new stuff" "name field"])
                                               :option-type :ok-cancel
                                               :success-fn (fn [e]
                                                             ;; create new active listbox
                                                             true))
                    ...
                    (-> (frame :title "manage todo lists"
                               ...
                               (top-bottom-split
                                ...
                                (border-panel :north "Hidden Todo Lists"
                                              ...
                                              :south (top-bottom-split
                                                      (left-right-split make-active delete-list)
                                                      done-button))
                                              :divider-location 1/2))
                        pack!
                        show!))))
